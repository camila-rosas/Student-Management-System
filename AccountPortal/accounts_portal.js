...
function guardFinanceAccess() {
  ...
  const allowedRoles = ['accountant', 'accounts', 'finance', 'financial', 'admin'];
...
function hydrateFinanceUser(user) {
  const displayName = user.name || 'Accounts User';
  const normalizedRole = String(user.role || '').toLowerCase();
  const roleLabel = normalizedRole === 'admin' ? 'Administrator' : 'Accounts Office';
...
function createNoticeSystem() {
  ...
  wraps.forEach(function (wrap, wrapIndex) {
    ...
    const storageKey = 'sms_finance_notices_' + wrapIndex;
...
function setupAccountSelection() {
  ...
  const activeRow = document.querySelector('.account-row.active');

  if (activeRow) {
    showDetail(activeRow.dataset.studentId);
  } else if (rows[0]) {
    showDetail(rows[0].dataset.studentId);
  }
}
