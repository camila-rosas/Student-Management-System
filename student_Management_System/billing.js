...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function renderBilling(account) {
    ...
    const estimatedHours = totalCharges > 0 ? ((totalCharges - 50) / 450).toFixed(0) : 0;

    sms.updateStatCard('Current Balance', sms.formatCurrency(currentBalance), currentBalance > 0 ? 'Balance due' : 'No balance due');
    sms.updateStatCard('Total Charges', sms.formatCurrency(totalCharges), `${estimatedHours} credit hours + fee`);
    sms.updateStatCard('Total Payments', sms.formatCurrency(totalPayments), 'Payments, scholarships & refunds');

    if (billingNote) {
      const dueDateText = account?.dueDate ? ` • Due date: ${account.dueDate}` : '';
      billingNote.innerHTML = `Current tuition rate: <strong>$450</strong> per credit hour and a <strong>$50</strong> student service fee${dueDateText}`;
    }
  ...
  function renderTransactions(transactions) {
    ...
    historyPanel.innerHTML = `
      ...
      <div class="section-subtitle">All charges, payments, and fees for Spring 2026</div>
    `;
      ...
      card.innerHTML = `
        <div class="course-top">
          <div>
            ...
            <div class="course-name">${amount < 0 ? '-' : ''}${sms.formatCurrency(Math.abs(amount))}</div>
          </div>
