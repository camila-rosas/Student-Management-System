...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (container) {
    container.addEventListener('click', async (event) => {
      ...
      if (enrollButton) await enrollInCourse(String(enrollButton.dataset.enrollId));
      if (dropButton) await dropCourse(String(dropButton.dataset.dropId));
    });
  }
  ...
  async function loadCatalog() {
    ...
    if (alertBar) {
      alertBar.textContent = 'ⓘ Course catalog is loaded from the Java backend. Enroll and drop changes update the database and every role automatically.';
    }
  ...
  function renderCatalog() {
    ...
    container.innerHTML = visibleCourses.map((course) => {
      return buildCourseCard(course, enrolledMap.get(course.courseId));
    }).join('');
  }
  ...
  async function enrollInCourse(courseId) {
    try {
      ...
      await loadCatalog();
      window.alert('Enrolled successfully. All dashboards and reports now reflect the change.');
    } catch (error) {
      console.error(error);
