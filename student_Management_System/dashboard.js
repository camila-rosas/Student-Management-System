...
function renderEnrolledCourses(registrations, sms) {
  ...
  Array.from(panel.children).forEach((child) => {
    if (child !== panelHead) child.remove();
  });

  if (!registrations.length) {
    sms.showInlineMessage(panel, 'No Courses Enrolled', 'You have no registered courses yet.', 's_course_catalog.html', 'Browse Course Catalog', '📖');
