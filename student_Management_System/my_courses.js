...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function updateSummary(registrations) {
    ...
    const totalHours = registrations.reduce((sum, registration) => sum + Number(registration.course?.courseHours || 0), 0);
  ...
  function renderCourses(registrations) {
    ...
    if (!registrations.length) {
      sms.showInlineMessage(panel, 'No Courses Enrolled', 'You have no registered courses this semester.', 's_course_catalog.html', 'Browse Course Catalog', '📖');
      return;
    }
    ...
    panel.querySelectorAll('[data-drop-id]').forEach((button) => {
      button.addEventListener('click', async () => {
        try {
          await sms.apiRequest(`/api/registration/drop/${button.dataset.dropId}`, { method: 'delete' });
